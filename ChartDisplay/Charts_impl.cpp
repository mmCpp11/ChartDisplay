...
namespace charts {
	...
	bool DoDownload(DoDownloadMode obj, const fs::path& outpath, const fs::path& exe_path, std::optional<std::chrono::year_month_day> airacdate) {
		const std::unordered_map<DoDownloadMode, std::wstring> do_map = {
			{DoDownloadMode::airac_dates,L"airac_dates"},{DoDownloadMode::charts,L"charts"}
		};
		Win64Wrapper::ProcCreationInfo pi;;
		std::wstring command_line(LR"("ChartDisplayDownloadHelper.exe" )");
		command_line += do_map.at(obj);
		command_line += L" ";
		if (obj == DoDownloadMode::airac_dates) {
			...
			if (!airacdate) {
				throw std::exception("For Charts date is required");
			}
		}
		command_line += outpath.native();
		command_line += L" ";
		if (obj == DoDownloadMode::charts) {
			auto cdatestr = std::format(L"{:%F}", airacdate.value());
			command_line += cdatestr;
		}
		auto ret = Win64Wrapper::CreateChildProcess(exe_path, command_line, pi,false,std::to_underlying(Win64Wrapper::ProcCreationFlags::NoConsoleNormal));
		if (ret.value() != 0) {
			if (ret.category() == std::system_category()) {
				Win64Wrapper::CreateMessageBox(std::format(L"Download Process Failed to execute with code {}.",ret.value()),L"Download Failure");
			}
			else {
				std::wstring msg = std::format(L"Download Process retuned non-zero exit code: {}", ret.value());
				Win64Wrapper::CreateMessageBox(msg.c_str(), L"Download Failure");
			}
			return false;
		}
		return true;
	...
	auto GetDatabaseHandle() {
		auto dbpath = Win64Wrapper::GetSysConfDefaultFilepath(Win64Wrapper::KnownFolderID::LocalAppData, true, "ChartDisplay", L"chartdisplay.sqlite");
		try {
			...
			else if (!fs::is_directory(dbpath.parent_path())) {
				throw;
			}
		}
		//specify database schema
		static auto db = sql::make_storage(dbpath.string(), sql::make_table("control",
			...
			sql::make_table("airac_dates", 
				sql::make_column("cycle", &AiracDates::AIRACInfo::cycle_id, sql::primary_key()),
				sql::make_column("date", &AiracDates::AIRACInfo::date), sql::check(sql::c(&AiracDates::AIRACInfo::cycle_id) >= 1000)));
		//try {
		//	db.sync_schema();
		//}
		//catch (std::system_error& e) {
		//	OutputDebugStringA(std::format("std::system_error. Error msg: {}, Error Code: {}", e.what(), e.code().value()).c_str());
		//	throw;
		//}
		db.sync_schema();
		return db;
	...
	AiracDates::AiracDates(std::optional<std::filesystem::path> downloader) :filedate_regex(R"((\d{4}-\d{2}-\d{2}))"),
		airacnumber_regex(R"(<strong>(\d{4})</strong>)"), airacdate_regex(R"(<strong>(\d{2}\s\w{3}\s\d{2})</strong>)"),
		downloader_path(downloader.value_or("ChartDisplayDownloadHelper.exe")) {
		if (downloader_path == "ChartDisplayDownloadHelper.exe") {
			auto exedirpath = Win64Wrapper::GetModuleFilepath().value_or(".").parent_path();
			downloader_path = exedirpath / "ChartDisplayDownloadHelper.exe";
		}
	...
	const AiracDates::AIRACInfo AiracDates::GetCurrentAiracCycle() {
		if (current.cycle_id == 0) {
			PopulateReferenceAIRAC();
		}
		return current;
	}
	const AiracDates::AIRACInfo AiracDates::GetPreviousAiracCycle() {
		if (previous.cycle_id == 0) {
			PopulateReferenceAIRAC();
		}
		return previous;
	}
	const AiracDates::AIRACInfo AiracDates::GetNextAiracCycle() {
		if (next.cycle_id == 0) {
			PopulateReferenceAIRAC();
		}
		return next;
	...
	void AiracDates::CommitData(bool control_only) {
		...
		auto db = GetDatabaseHandle();
		auto chart_update_rec = db.get<ControlRecord>(6);
		chart_update_rec.pid = -1; //not really necessary but to match the others
		auto guard = db.transaction_guard();
	...
	FAAChartProcessor::FAAChartProcessor(std::optional<std::filesystem::path> downloader)
		:chartdir(Win64Wrapper::GetSysConfDefaultFilepath(Win64Wrapper::KnownFolderID::LocalAppData,true,L"ChartDisplay\\Charts")),
		manchartxml(chartdir.parent_path() / "custom_charts.xml"), downloader_path(downloader.value_or("ChartDisplayDownloadHelper.exe")),
		current_cycle(downloader_path){
	...
	bool FAAChartProcessor::UpdateCharts(bool no_download, bool force) {
		...
		bool check = false;
		auto trigger_download = [this](const fs::path& tempdir,std::chrono::year_month_day cdate) {
			CleanCharts(tempdir);
			try {
				fs::create_directories(tempdir);
			}
			...
			if (DoDownload(DoDownloadMode::charts, tempdir, downloader_path, cdate)) {
				return true;
			}
			else {
				return false;
	...
	void FAAChartProcessor::GetChartsAndOrganize(const std::filesystem::path& tempdir) {
		...
		for (auto& row : csvs.at(clsindex)) {
			...
			if (airspace != 'E') {
				recmap.at(row["ARPT_ID"].get<std::string>()).airspace_class = airspace;
			}
		}
			...
			for (auto& city : state.children(nodes[2]))
			{
				for (auto& airport : city.children(nodes[3]))
				{
					...
					for (auto& c : airport.children(nodes[4]))
					{
						...
						newchartpath /= c.child_value("pdf_name"); //filename
